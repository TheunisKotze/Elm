...
reorder (A region expression) =
    ...
    case expression of
      ...
      Extras port ->
          Extras <$>
          case port of
            V.PortOut name st signal -> V.PortOut name st <$> reorder signal

            V.PortIn name st -> return $ V.PortIn name st
...
buildDefDict defs =
  ...
  where
    addKey :: [(V.CanonicalDef, [String])] -> [(V.CanonicalDef, Int, [String])]
    addKey = zipWith (\n (pdef,deps) -> (pdef,n,deps)) [0..]

    variableToKey :: (V.CanonicalDef, Int, [String]) -> [(String, Int)]
    variableToKey (V.Definition pattern _ _, key, _) =
        [ (var, key) | var <- Set.toList (P.boundVars pattern) ]

    variableToKeyMap :: [(V.CanonicalDef, Int, [String])] -> Map.Map String Int
    variableToKeyMap pdefsDeps =
        Map.fromList (concatMap variableToKey pdefsDeps)

    realDeps :: [(V.CanonicalDef, Int, [String])] -> [(V.CanonicalDef, Int, [Int])]
